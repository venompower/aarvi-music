...
from AARVIMUSIC import YouTube, app
from AARVIMUSIC.core.call import AARVI
from AARVIMUSIC.misc import SUDOERS, db
...
from AARVIMUSIC.utils.formatters import seconds_to_min
...
async def del_back_playlist(client, CallbackQuery, _):
    ...
    if command == "Pause":
        ...
        await AARVI.pause_stream(chat_id)
        buttons = [
            [
                InlineKeyboardButton(
        ...
        await AARVI.resume_stream(chat_id)
        buttons_resume = [
            [
                InlineKeyboardButton(
        ...
        await CallbackQuery.answer()
        await AARVI.stop_stream(chat_id)
        ...
        await AARVI.mute_stream(chat_id)
        ...
        await AARVI.unmute_stream(chat_id)
        ...
        if command == "Skip":
            ...
            try:
                ...
                if not check:
                    ...
                    try:
                        return await AARVI.stop_stream(chat_id)
                    except:
                        return
            except:
                try:
        ...
        if "live_" in queued:
            ...
            try:
                ...
                image = None
            try:
                await AARVI.skip_stream(chat_id, link, video=status, image=image)
            except:
            ...
            try:
                ...
                image = None
            try:
                await AARVI.skip_stream(chat_id, file_path, video=status, image=image)
            except:
                return await mystic.edit_text(_["call_6"])
            ...
            try:
                await AARVI.skip_stream(chat_id, videoid, video=status)
            except:
                return await CallbackQuery.message.reply_text(_["call_6"])
            button = stream_markup2(_, chat_id)
            run = await CallbackQuery.message.reply_photo(
            ...
            if videoid == "telegram":
                ...
                try:
                    ...
                    image = None
            try:
                await AARVI.skip_stream(chat_id, queued, video=status, image=image)
            except:
                return await CallbackQuery.message.reply_text(_["call_6"])
            if videoid == "telegram":
                button = stream_markup2(_, chat_id)
